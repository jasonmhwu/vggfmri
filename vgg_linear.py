...
preN = '../vgg_places_results/'+roiName[targetROI]+layername
postN = ''
#np.save(preN+'seq'+postN, randSeq)
f_trn_acc = np.memmap(preN+'trn'+postN, dtype=trn_acc.dtype,shape=trn_acc.shape, mode='w+')
f_acc = np.memmap(preN+'acc'+postN, dtype=acc.dtype,shape=acc.shape, mode='w+')
...
#Parallel(n_jobs = 40)(delayed(modelEstimate)(j, f_trn_acc, f_acc, bestPred, bestTrain) for j in range(nb_classes))
Parallel(n_jobs = 40)(delayed(modelEstimate)(j) for j in range(800, nb_classes))
print('segment time is '+str(time.time()-t))
...
'''
	tmpMax = -200
	for i in xrange(len(alpha)):
		#print ('model at alpha = ', alpha[i], ' at feature ', j)
		...
		#print(valscore)
		#print(trnscore)
		f_trn_acc[i, j] = trnscore
		f_acc[i, j] = valscore
		if valscore > tmpMax:
			tmpMax = valscore
			bestPred[:,j] = y_pred
			bestTrain[:,j] = y_trn
	print('feature '+str(j)+' has valscore '+"{0:.3f}".format(tmpMax))
'''
